...
class Auditory:
    ...
    def update_audit(self, 
                    start_date: int, start_time:int ,
                    end_date:int, end_time:int, 
                    process_number: str, 
                    data: int, 
        ...
        if verify_procces_executed and data != 0:
            ...
            df_executed = pd.DataFrame(self.cursor_rino
                                        .fetchall(), columns=columns)
            df_executed = df_executed.loc[df_executed.start_date == int(start_date)]
            df_executed.sort_values(by = ['start_date', 'start_time'], 
                                    ascending = False, 
                                    inplace = True)
            ...
            if len(df_executed) != 0:
                ...
                if (((data_precessed == data) or (np.isnan(data_precessed))) 
                    and (data_precessed != 0)):
                    data = 'NULL'
